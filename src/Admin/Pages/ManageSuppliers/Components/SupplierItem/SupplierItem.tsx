...
export default function CategoryItem({
  ...
  handleEditItem: (item: SupplierItemType) => void
  maxIndex: number
  index: number
}) {
  ...
  return (
    <div
      className={`${index % 2 === 0 ? "bg-[#f2f2f2]" : "bg-white"} dark:bg-darkPrimary grid grid-cols-10 items-center gap-2 py-3 cursor-pointer border-t-0 border border-[#dedede] dark:border-darkBorder px-4 ${index + 1 === maxIndex ? "rounded-bl-lg rounded-br-lg" : ""}`}
      key={item._id}
    >
      ...
      <div className="col-span-1 break-words text-black dark:text-white">{item.contactName}</div>
      <div className="col-span-1 break-words text-black dark:text-white">{item.email}</div>
      <div className="col-span-1 text-black dark:text-white">{item.phone}</div>
